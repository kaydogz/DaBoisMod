...
public class DBMLakeFeature extends Feature<BlockStateFeatureConfig> {

	private static final BlockState AIR = Blocks.CAVE_AIR.getDefaultState();

	public DBMLakeFeature(Function<Dynamic<?>, ? extends BlockStateFeatureConfig> function) {
		super(function);
	}
						...
						for(int j4 = 4; j4 < 8; ++j4) {
							if (aboolean[(i2 * 16 + j3) * 8 + j4]) {
								...
								if (isDirt(worldIn.getBlockState(blockpos).getBlock()) && worldIn.getLightFor(LightType.SKY, pos.add(i2, j4, j3)) > 0) {
									...
									} else if (biome.getSurfaceBuilderConfig().getTop().getBlock() == Blocks.GRASS_BLOCK) {
										worldIn.setBlockState(blockpos, Blocks.GRASS_BLOCK.getDefaultState(), 2); // Change this when creating new dirt/grass blocks.
									} else {
										worldIn.setBlockState(blockpos, DBMBlocks.BOTSWANIAN_GRASS_BLOCK.get().getDefaultState(), 2);
									}
								}
							}
						}
